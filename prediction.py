...
def afficher_prediction():
    ...
    # Titre principal
    st.markdown(
    '<h1 style="font-size: 2.5rem; font-weight: bold; text-align: center; color: #FFD700; background-color: #000000; padding: 10px; border-radius: 10px;">🔮 Prédiction de la consommation de CO2</h1>',
    unsafe_allow_html=True,
)
    ...
    with tab2:
        ...
        if uploaded_file:
            ...
            if uploaded_file.name.endswith('.csv'):
                data_excel = pd.read_csv(uploaded_file)
            else:
            ...
            if st.button("Lancer les prédictions multiples"):
                ...
                csv = data_excel.to_csv(index=False).encode('utf-8')
                data_excel.to_excel("results.xlsx", index=False, engine='openpyxl')
